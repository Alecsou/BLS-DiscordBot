...
module.exports = async (client, interaction) => {
    if (!interaction.isModalSubmit() || interaction.customId!==`nether-a-surveiller-${interaction.user.id}`) return;
    ...
    const newembed = EmbedBuilder.from(embed.toJSON()).addFields({name:"Nether",value:`**A surveiller : ** \`${pseudo}\` (${comment})`});
